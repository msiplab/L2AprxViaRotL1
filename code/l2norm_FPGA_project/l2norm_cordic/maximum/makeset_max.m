%% model name
for i = 27:32
%     sys = "cordic_max_fixpt8_19/total19/Subsystem"+num2str(i)+"bit";
%     sys = "cordic_max_fixpt20_26/total26/Subsystem"+num2str(i)+"bit";
    sys = "cordic_max_fixpt27_32/total32/Subsystem"+num2str(i)+"bit";
%     sys = 'cordic_max_16/Subsystem16bit';
    ...
    set_param(sys+"/CORDIC16", 'pipelining_mode', 'Maximum')
...
for i = 8:19
    for j = 1:length(fin)
        if fin(j) == "out"+num2str(i)
            s = get(out,char(fin(j))).signals.values(21:120);
            me(i-7) = sum(s)/100;
        end
    end
end
...
%     for j = 1:length(fin)
%         if fin(j) == "out"+num2str(i)
%             s = get(out,char(fin(j))).signals.values(28:127);
%             me(i-19) = sum(s)/100;
%         end
%     end
% end
...
%     for j = 1:length(fin)
%         if fin(j) == "out"+num2str(i)
%             s = get(out,char(fin(j))).signals.values(34:133);
%             me(i-26) = sum(s)/100;
%         end
%     end
% end
...
%% ketugou
cord_max = [cord_max8_19,cord_max20_26,cord_max27_32];
