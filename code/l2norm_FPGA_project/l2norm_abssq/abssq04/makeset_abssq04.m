%% model name
for k = 16
    %sys = "abssq04_fixpt8_32/total32/Subsystem"+num2str(k)+"bit";
    sys = 'abssq04_fixpt16/Subsystem16bit';
    ...
    set_param(sys+"/Slice1", 'mode', 'Two Bit Locations')
    set_param(sys+"/Slice1", 'bit1', '-1')
    set_param(sys+"/Slice1", 'base1', 'MSB of Input')
    set_param(sys+"/Slice1", 'bit0', num2str(k-1))
    set_param(sys+"/Slice1", 'base0', 'LSB of Input')
    set_param(sys+"/Scale1", 'scale_factor', num2str(-(k-1)))

    set_param(sys+"/Slice2", 'nbits', num2str(k-1))
    set_param(sys+"/Slice2", 'mode', 'Upper Bit Location + Width')
    set_param(sys+"/Slice2", 'bit1', '-3')
    set_param(sys+"/Slice2", 'base1', 'MSB of Input')
    set_param(sys+"/Scale2", 'scale_factor', num2str(-(k-2)))

    set_param(sys+"/Slice3", 'mode', 'Two Bit Locations')
    set_param(sys+"/Slice3", 'bit1', '-5')
    set_param(sys+"/Slice3", 'base1', 'MSB of Input')
    set_param(sys+"/Slice3", 'bit0', num2str(k-2))
    set_param(sys+"/Slice3", 'base0', 'LSB of Input')
    set_param(sys+"/Scale3", 'scale_factor', num2str(-(k-1)))
